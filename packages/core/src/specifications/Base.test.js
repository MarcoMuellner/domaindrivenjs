...
describe("specification", () => {
  ...
  describe("basic validation", () => {
    it("should throw error if options are missing", () => {
      expect(() => specification()).toThrow(
        "Specification options are required",
      );
    });

    it("should throw error if name is missing", () => {
      expect(() => specification({ isSatisfiedBy: () => true })).toThrow(
        "Specification name is required",
      );
    });

    it("should throw error if isSatisfiedBy is missing", () => {
      expect(() => specification({ name: "Test" })).toThrow(
        "Specification must have an isSatisfiedBy function",
      );
    });

    it("should throw error if isSatisfiedBy is not a function", () => {
      expect(() =>
        specification({ name: "Test", isSatisfiedBy: "not a function" }),
      ).toThrow("Specification must have an isSatisfiedBy function");
    });
  });
  ...
  describe("logical operations", () => {
    ...
    describe("complex operations", () => {
      it("should handle complex combinations of AND, OR, and NOT", () => {
        ...
        // Test name
        expect(complexSpec.name).toBe(
          "IsPositive AND IsEven OR NOT IsLessThan10",
        );
