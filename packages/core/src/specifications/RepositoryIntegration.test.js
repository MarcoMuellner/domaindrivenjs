...
describe('Specification Repository Integration', () => {
    ...
    // Sample data
    const products = [
        { id: 'prod-1', name: 'Budget Laptop', price: 499.99, category: 'electronics', tags: ['budget', 'laptop'], inStock: true },
        { id: 'prod-2', name: 'Premium Smartphone', price: 999.99, category: 'electronics', tags: ['premium', 'smartphone'], inStock: true },
        { id: 'prod-3', name: 'Ergonomic Chair', price: 299.99, category: 'furniture', tags: ['office', 'ergonomic'], inStock: false },
        { id: 'prod-4', name: 'Wireless Headphones', price: 149.99, category: 'electronics', tags: ['audio', 'wireless'], inStock: true },
        { id: 'prod-5', name: 'Coffee Table', price: 199.99, category: 'furniture', tags: ['living room'], inStock: true }
    ];
    ...
    describe('findBySpecification', () => {
        ...
        it('should find aggregates using a combined specification', async () => {
            ...
            // Act
            const results = await productRepository.findBySpecification(expensiveElectronics);
        ...
        it('should find aggregates using OR specifications', async () => {
            ...
            const inStockElectronicsOrFurniture = inStock.and(electronics.or(furniture));

            // Act
            const results = await productRepository.findBySpecification(inStockElectronicsOrFurniture);

            // Assert
            expect(results.length).toBe(4); // All in-stock products
            expect(results.every(product => product.inStock)).toBe(true);
            expect(results.every(product =>
        ...
        it('should find aggregates using NOT specifications', async () => {
            ...
            expect(results.every(product => product.category !== 'furniture')).toBe(true);
        });
        ...
        it('should find aggregates using custom specifications', async () => {
            ...
            // Act
            const results = await productRepository.findBySpecification(PremiumProduct);
        ...
        it('should find aggregates using array specifications', async () => {
            ...
            // Act
            const results = await productRepository.findBySpecification(hasWirelessTag);
        ...
        it('should find aggregates using propertyIn specification', async () => {
            // Arrange
            const selectedCategories = propertyIn('category', ['electronics', 'appliances']);

            // Act
            const results = await productRepository.findBySpecification(selectedCategories);
    ...
    describe('count with specifications', () => {
        it('should count aggregates using specifications', async () => {
            ...
            const electronicsCount = await productRepository.count({ category: 'electronics' });
