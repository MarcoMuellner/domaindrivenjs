...
describe("InMemory adapter", () => {
  ...
  it("should throw error if identity field is missing", () => {
    expect(() => createInMemoryAdapter({})).toThrow(
  ...
  describe("findBySpecification", () => {
    ...
    it("should throw error if specification is invalid", async () => {
      // Act & Assert
      await expect(adapter.findBySpecification({})).rejects.toThrow(
        "Invalid specification",
      );
      await expect(adapter.findBySpecification(null)).rejects.toThrow(
        "Invalid specification",
      );
    });
  });
  ...
  describe("save", () => {
    ...
    it("should throw error if aggregate is missing identity field", async () => {
      ...
      await expect(adapter.save({ name: "No ID" })).rejects.toThrow(
        "Aggregate missing identity field",
      );
    });
  ...
  describe("saveAll", () => {
    ...
    it("should throw error if aggregates is not an array", async () => {
      // Act & Assert
      await expect(adapter.saveAll("not-an-array")).rejects.toThrow(
        "Aggregates must be an array",
      );
      await expect(adapter.saveAll(null)).rejects.toThrow(
        "Aggregates must be an array",
      );
    });
  });
