...
describe('repository', () => {
    ...
    const TestAggregate = aggregate({
        ...
        methods: {
            ...
            emitTestEvent() {
                ...
                return TestAggregate.update(this, {})
                    .emitEvent('TestEvent', {
                        aggregateId: this.id,
                        name: this.name
                    });
    ...
    beforeEach(() => {
        ...
        const testData = [
            TestAggregate.create({ id: 'test-1', name: 'Test 1', value: 100, category: 'A' }),
            TestAggregate.create({ id: 'test-2', name: 'Test 2', value: 200, category: 'B' }),
            TestAggregate.create({ id: 'test-3', name: 'Test 3', value: 50, category: 'A' }),
            TestAggregate.create({ id: 'test-4', name: 'Test 4', value: 150, category: 'B' })
        ];
    ...
    describe('findById', () => {
        ...
        it('should throw RepositoryError if ID is missing', async () => {
            ...
            await expect(testRepository.findById('')).rejects.toThrow(RepositoryError);
            await expect(testRepository.findById(null)).rejects.toThrow(RepositoryError);
        });
        ...
        it('should wrap adapter errors in RepositoryError', async () => {
            ...
            // Act & Assert
            await expect(errorRepository.findById('test-1')).rejects.toThrow(RepositoryError);
    ...
    describe('findByIds', () => {
        it('should find multiple aggregates by IDs', async () => {
            // Act
            const resultMap = await testRepository.findByIds(['test-1', 'test-2', 'non-existent']);
        ...
        it('should throw RepositoryError if IDs parameter is not an array', async () => {
            // Act & Assert
            await expect(testRepository.findByIds('test-1')).rejects.toThrow(RepositoryError);
            await expect(testRepository.findByIds(null)).rejects.toThrow(RepositoryError);
            await expect(testRepository.findByIds({})).rejects.toThrow(RepositoryError);
        });
    });
    ...
    describe('exists', () => {
        ...
        it('should throw RepositoryError if ID is missing', async () => {
            ...
            await expect(testRepository.exists('')).rejects.toThrow(RepositoryError);
            await expect(testRepository.exists(null)).rejects.toThrow(RepositoryError);
        });
    });
    ...
    describe('findAll', () => {
        ...
        it('should return empty array if no aggregates match filter', async () => {
            // Act
            const results = await testRepository.findAll({ category: 'Non-existent' });
    ...
    describe('findOne', () => {
        ...
        it('should throw RepositoryError if filter is missing', async () => {
            ...
            await expect(testRepository.findOne({})).rejects.toThrow(RepositoryError);
            await expect(testRepository.findOne(null)).rejects.toThrow(RepositoryError);
        });
    });
    ...
    describe('findBySpecification', () => {
        it('should find aggregates using specification object', async () => {
            // Act
            const results = await testRepository.findBySpecification(HighValueSpecification);
        ...
        it('should throw RepositoryError if specification is missing', async () => {
            // Act & Assert
            await expect(testRepository.findBySpecification()).rejects.toThrow('Specification is required');
            await expect(testRepository.findBySpecification(null)).rejects.toThrow('Specification is required');
        });

        it('should throw error if specification is invalid', async () => {
            // Act & Assert
            await expect(testRepository.findBySpecification({})).rejects.toThrow('Invalid specification');
        });
    });
    ...
    describe('saveAll', () => {
        ...
        it('should throw RepositoryError if aggregates parameter is not an array', async () => {
            // Act & Assert
            await expect(testRepository.saveAll('not-an-array')).rejects.toThrow(RepositoryError);
            await expect(testRepository.saveAll({})).rejects.toThrow(RepositoryError);
            await expect(testRepository.saveAll(null)).rejects.toThrow(RepositoryError);
        });
    ...
    describe('delete', () => {
        ...
        it('should throw RepositoryError if ID is missing', async () => {
            ...
            await expect(testRepository.delete('')).rejects.toThrow(RepositoryError);
            await expect(testRepository.delete(null)).rejects.toThrow(RepositoryError);
        });
    });
        ...
        it('should use adapter implementation for findByIds if available', async () => {
            ...
            const specialAdapter = {
                ...
                findByIds: vi.fn().mockResolvedValue(new Map([
                    ['test-1', { id: 'test-1', name: 'Test 1', value: 100 }]
                ]))
            };
        ...
        it('should use adapter implementation for findBySpecification if available', async () => {
            ...
            const specialAdapter = {
                ...
                findBySpecification: vi.fn().mockResolvedValue([
                    { id: 'test-2', name: 'Test 2', value: 200 }
                ])
            };
            ...
            // Assert
            expect(specialAdapter.findBySpecification).toHaveBeenCalledWith(HighValueSpecification);
            expect(specialAdapter.findAll).not.toHaveBeenCalled();
        });
