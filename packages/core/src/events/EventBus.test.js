...
describe("EventBus", () => {
  ...
  describe("publishing events", () => {
    ...
    it("should throw error when publishing an invalid event", async () => {
      // Arrange & Act & Assert
      await expect(eventBus.publish(null)).rejects.toThrow(EventBusError);

      await expect(eventBus.publish({})).rejects.toThrow(EventBusError);
  ...
  describe("subscribing to events", () => {
    ...
    it("should throw error when subscribing with invalid handler", () => {
      // Arrange & Act & Assert
      expect(() => eventBus.on("TestEvent", null)).toThrow(EventBusError);

      expect(() => eventBus.on("TestEvent", "not-a-function")).toThrow(
        EventBusError,
      );
    });
    ...
    it("should throw error when subscribing with invalid event type", () => {
      ...
      // Act & Assert
      expect(() => eventBus.on(null, handler)).toThrow(EventBusError);

      expect(() => eventBus.on({}, handler)).toThrow(EventBusError);

      expect(() => eventBus.on({ name: "InvalidEvent" }, handler)).toThrow(
        EventBusError,
      );
    });
  ...
  describe("custom adapters", () => {
    ...
    it("should validate custom adapter", () => {
      // Arrange & Act & Assert
      expect(() => eventBus.setAdapter(null)).toThrow(EventBusError);

      expect(() => eventBus.setAdapter({})).toThrow(EventBusError);

      expect(() => eventBus.setAdapter({ publish: () => {} })).toThrow(
        EventBusError,
      );

      expect(() => eventBus.setAdapter({ subscribe: () => {} })).toThrow(
        EventBusError,
      );
    });
  });
